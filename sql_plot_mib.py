        ...
        def make_document(doc):
                    ...
                    if condition in data_by_condition and condition in scatter_sources:
                        ...
                        if renderer.visible:
                            ...
                            if has_ringratio and ringratio_min is not None and ringratio_max is not None:
                                # Only include valid Ring Ratio values (positive numbers)
                                mask = mask & (df['ringratio'] > 0) & (df['ringratio'] >= ringratio_min) & (df['ringratio'] <= ringratio_max)
            ...
            def recalculate_kde():
                ...
                if has_timestamp and timestamp_slider:
                    timestamp_min, timestamp_max = timestamp_slider.value
                    filters['rel_timestamp'] = (timestamp_min, timestamp_max)
                    
                if has_ringratio and ringratio_slider:
                    ringratio_min, ringratio_max = ringratio_slider.value
                        ...
                        if len(df) > 5:  # Need enough points for KDE
                            try:
                                ...
                                if condition in scatter_sources:
                                    ...
                                    # Get indices mapping between filtered data and current data
                                    # This assumes that the ordering is maintained
                                    if len(current_data['area']) == len(df):
                                        current_data['density'] = df['density'].values
            ...
            if has_ringratio and ringratio_plot:
                try:
                    ...
                    try:
                        # Try the older Panel approach
                        tabs = Tabs(tabs=[
                            Panel(child=scatter_plot, name="scatter_plot", title="Scatter Plot"),
                            Panel(child=ringratio_plot, name="histogram", title="Ring Ratio Histogram")
                        ])
                    except Exception as e:
                        print(f"Error creating tabs: {e}")
                        final_layout = layout([[controls, scatter_plot]])
...
def main():
    ...
    parser.add_argument('--server', action='store_true', help='Run in Bokeh server mode for interactive KDE recalculation')
